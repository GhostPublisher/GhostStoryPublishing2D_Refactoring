...
장점 :
 - 획일화된 객체 관리가 가능함.
 - Main 기반 형식과 매우 유사하여, 전반적인 게임 코드 체계 이해에 도움이 많이 됨.
 - MonoBehaviour의 사용과 Unity 이벤트 함수 사용을 최소화 하는 것이 자연스러움.

단점 :
 - 프로젝트 진행 속도 '독립지연참조' 형식보다 상대적으로 늦음. ( 객체 Load & Bind 작업이 추가됨 ) <- 가장 큰 문제 ( 느려짐 )
 - 익숙해지기 전까지 'Load + Bind 작업'이 매우 번거로움.
 - 참조 관계를 미리 정의하는 것에 의해, 수정과 개선에 매우 취약함.

 - 고민하고 진행될 수록, Unity의 GameObject와 Component의 의도와 장점을 없앤다는 느낌이 듬.

ps. 현재까지 해당 프로젝트는 이름과 달리, 코드 체계에 대한 연구를 주로 진행하였습니다.
 - 이후, 리펙토링과 코드 체계 연구를 분리할 계획입니다.
...
3. 기존 아키텍처에 적용하기 위한 Entity 정의
 -  둘 중 어떠한 형태를 사용할지 고민 중 입니다. ( 현재는 노출 최소형 Entity가 유니티에 더 적합하다고 생각합니다. )
